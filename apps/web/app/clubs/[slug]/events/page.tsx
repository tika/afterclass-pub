...
function EventsPageContent() {
  ...
  const handleEdit = (event: (typeof events)[0]) => {
    router.push(`/clubs/${slug}/events/${event.event.id}/edit`);
  };
  ...
  const handleDuplicate = (event: (typeof events)[0]) => {
    ...
    router.push(`/clubs/${slug}/events/create?${params.toString()}`);
  };
  ...
  const handleMapEventClick = (mapEvent: (typeof mapEvents)[0]) => {
    ...
    if (fullEvent) {
      router.push(`/clubs/${slug}/events/${fullEvent.event.id}/edit`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push(`/clubs/${slug}/events/create`)}>
