...
export function CreateEventForm({ eventId }: { eventId?: string }) {
  ...
  // Get groupId from URL params, localStorage, or event data
  useEffect(() => {
    if (isEditMode && eventData) {
      setGroupId(eventData.event.groupId);
    } else {
      const urlGroupId = searchParams.get("groupId");
      const storedGroupId = localStorage.getItem("selectedGroupId");
      setGroupId(urlGroupId || storedGroupId);
    }
  }, [isEditMode, eventData, searchParams]);
  ...
  const createEventMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/dashboard/events?groupId=${groupId}`);
    },
    onError: (error: Error) => {
